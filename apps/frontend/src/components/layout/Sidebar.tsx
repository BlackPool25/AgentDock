...
import { LayoutDashboard, Workflow, Settings, Wifi, WifiOff } from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <aside className="w-14 flex flex-col items-center py-4 gap-4 border-r border-border bg-card">
      <div className="text-primary font-bold text-lg">AD</div>
      {links.map(({ to, icon: Icon, label }) => (
        <Link
          key={to}
          to={to}
          title={label}
          className={`p-2 rounded-lg ${pathname === to ? "bg-primary/20 text-primary" : "text-muted-foreground hover:text-foreground"}`}
        >
          <Icon size={20} />
        </Link>
      ))}
